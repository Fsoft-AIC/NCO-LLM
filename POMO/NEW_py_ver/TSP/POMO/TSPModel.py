...
class TSP_Decoder(nn.Module):
    ...
    def forward(self, encoded_last_node, ninf_mask):
        ...
        mh_atten_out = self.multi_head_combine(out_concat) # q^T(c)
        # shape: (batch, pomo, embedding)

        #  Single-Head Attention, for probability calculation
        #######################################################
        score = torch.matmul(mh_atten_out, self.single_head_key) # q^T(c)*k__j
        # shape: (batch, pomo, problem)

        sqrt_embedding_dim = self.model_params['sqrt_embedding_dim'] #sqrt(d_k)
        logit_clipping = self.model_params['logit_clipping'] #C

        score_scaled = score / sqrt_embedding_dim
        # shape: (batch, pomo, problem)

        score_clipped = logit_clipping * torch.tanh(score_scaled) #u(c)j

        score_masked = score_clipped + ninf_mask

        probs = F.softmax(score_masked, dim=2) #p_i = p_\theta(\pi_t=i|...)
        # shape: (batch, pomo, problem)
