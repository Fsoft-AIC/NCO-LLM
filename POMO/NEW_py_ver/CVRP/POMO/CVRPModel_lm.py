...
class CVRPModel(nn.Module):
    ...
    def pre_forward(self, reset_state):
        ...
        ######################## Add LLM-design attention bias #############################
        if ATTENTION_BIAS:
            self.attention_bias = torch.stack([
                heuristics1(distance_matrices[i], all_node_demands[i]) for i in range(all_nodes_xy.size(0))
            ], dim=0)
    ...
    def forward(self, state):
        ...
        if state.selected_count == 0:  # First Move, depot
            ...
            # Add LLM-design attention bias impl
