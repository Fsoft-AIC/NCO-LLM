...
class TSPModel(nn.Module):
    ...
    def pre_forward(self, reset_state):
        ...
        distance_matrices = get_distance_matrix(reset_state.problems)

        ######################## LLM component #############################
        if ATTENTION_BIAS:
            self.attention_bias = torch.stack([
                heu.heuristics(distance_matrices[i]) for i in range(distance_matrices.size(0))
            ], dim=0)
    ...
    def forward(self, state):
        ...
        if state.current_node is None:
            ...
            # shape: (batch, pomo, embedding)
            # LLM component: get attention bias for the current node
            attention_bias_current_node = self.attention_bias[torch.arange(batch_size)[:, None], state.current_node, :] if ATTENTION_BIAS else None
...
class TSP_Decoder(nn.Module):
    ...
    def forward(self, encoded_last_node, ninf_mask, attention_bias_current_node):
        ...
        # LLM component: reshape the attention score
        score = score + attention_bias_current_node if ATTENTION_BIAS else score
