...
class TSPEnv:
    ...
    def load_problems(self, episode, batch_size):
        ...
        if self.problem_size == 100:
            ...
            self.problems, self.solution = self.raw_data_nodes1[episode:episode + batch_size], self.raw_data_tours1[episode:episode + batch_size]
        elif self.problem_size == 200:
            self.problems, self.solution = self.raw_data_nodes2[episode:episode + batch_size], self.raw_data_tours2[episode:episode + batch_size]
    ...
    def shuffle_data(self):
        ...
        self.raw_data_tours1 = self.raw_data_tours1[index]
        self.raw_data_nodes2 = self.raw_data_nodes2[index]
        self.raw_data_tours2 = self.raw_data_tours2[index]
    ...
    def load_raw_data(self, episode,begin_index=0):
        ...
        self.raw_data_nodes = []
        self.raw_data_tours = []
        for line in tqdm(open('/cm/shared/ML4CO/data/TSP/train_TSP100_n100w.txt', "r").readlines()[0+begin_index:episode+begin_index], ascii=True):
            line = line.split(" ")
        ...
        self.raw_data_nodes1 = []
        self.raw_data_tours1 = []
        for line in tqdm(open('/home/code/NAT/azure_code/LEHD/TSP/datasets/10k_TSP_150_instances1.txt', "r").readlines()[0+begin_index:episode+begin_index], ascii=True):
            line = line.split(" ")
            num_nodes = int(line.index('output') // 2)
            nodes = [[float(line[idx]), float(line[idx + 1])] for idx in range(0, 2 * num_nodes, 2)]

            self.raw_data_nodes1.append(nodes)
            tour_nodes = [int(node) - 1 for node in line[line.index('output') + 1:-1]]

            self.raw_data_tours1.append(tour_nodes)

        self.raw_data_nodes1 = torch.tensor(self.raw_data_nodes1,requires_grad=False)
        self.raw_data_tours1 = torch.tensor(self.raw_data_tours1,requires_grad=False)
        
        self.raw_data_nodes2 = []
        self.raw_data_tours2 = []
        for line in tqdm(open('/home/code/NAT/azure_code/LEHD/TSP/datasets/all_TSP_200_instances.txt', "r").readlines()[0+begin_index:episode+begin_index], ascii=True):
            line = line.split(" ")
            num_nodes = int(line.index('output') // 2)
            nodes = [[float(line[idx]), float(line[idx + 1])] for idx in range(0, 2 * num_nodes, 2)]
