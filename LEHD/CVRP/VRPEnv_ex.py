...
class VRPEnv:
    ...
    def load_problems(self, episode, batch_size, ):
        ...
        if self.problem_size == 100:
            ...
            self.problems_nodes = self.raw_data_nodes1[episode:episode + batch_size]
            # shape (B,V+1,2)
            self.Batch_demand = self.raw_data_demand1[episode:episode + batch_size]
            # shape (B,V+1)

            self.Batch_capacity = self.raw_data_capacity1[episode:episode + batch_size]

            self.solution = self.raw_data_node_flag1[episode:episode + batch_size]
            # shape (B,V,2)
        elif self.problem_size == 200:
            self.problems_nodes = self.raw_data_nodes2[episode:episode + batch_size]
            # shape (B,V+1,2)
            self.Batch_demand = self.raw_data_demand2[episode:episode + batch_size]
            # shape (B,V+1)

            self.Batch_capacity = self.raw_data_capacity2[episode:episode + batch_size]

            self.solution = self.raw_data_node_flag2[episode:episode + batch_size]
            # shape (B,V,2)
    ...
    def shuffle_data(self):
        ...
        self.raw_data_nodes1 = self.raw_data_nodes1[index]
        self.raw_data_capacity1 = self.raw_data_capacity1[index]
        self.raw_data_demand1 = self.raw_data_demand1[index]
        self.raw_data_cost1 = self.raw_data_cost1[index]
        self.raw_data_node_flag1 = self.raw_data_node_flag1[index]
        
        self.raw_data_nodes2 = self.raw_data_nodes2[index]
        self.raw_data_capacity2 = self.raw_data_capacity[index]
        self.raw_data_demand2 = self.raw_data_demand2[index]
        self.raw_data_cost2 = self.raw_data_cost2[index]
        self.raw_data_node_flag2 = self.raw_data_node_flag2[index]
    ...
    def load_raw_data(self,episode=1000000):
        ...
        if self.env_params['mode']=='train':
            ...
            self.raw_data_nodes1 = []
            self.raw_data_capacity1 = []
            ...
            for line in tqdm(open('/home/code/NAT/azure_code/LEHD/CVRP/data/vrp150_test_n5000_example.txt', "r").readlines()[0:episode], ascii=True):
                ...
                self.raw_data_nodes1.append(loc)
                self.raw_data_capacity1.append(capacity)
                self.raw_data_demand1.append(demand)
                self.raw_data_cost1.append(cost)
                self.raw_data_node_flag1.append(node_flag)

            self.raw_data_nodes1 = torch.tensor(self.raw_data_nodes1, requires_grad=False)
            # shape (B,V+1,2)  customer num + depot
            self.raw_data_capacity1 = torch.tensor(self.raw_data_capacity1, requires_grad=False)
            # shape (B )
            self.raw_data_demand1 = torch.tensor(self.raw_data_demand1, requires_grad=False)
            # shape (B,V+1) customer num + depot
            self.raw_data_cost1 = torch.tensor(self.raw_data_cost1, requires_grad=False)
            # shape (B )
            self.raw_data_node_flag1 = torch.tensor(self.raw_data_node_flag1, requires_grad=False)
            # shape (B,V,2)
            self.raw_data_nodes2 = []
            self.raw_data_capacity2 = []
            ...
            for line in tqdm(open('/home/code/NAT/azure_code/LEHD/CVRP/data/vrp200_test_n5000_example.txt', "r").readlines()[0:episode], ascii=True):
                ...
                self.raw_data_nodes2.append(loc)
                self.raw_data_capacity2.append(capacity)
                self.raw_data_demand2.append(demand)
                self.raw_data_cost2.append(cost)
                self.raw_data_node_flag2.append(node_flag)

            self.raw_data_nodes2 = torch.tensor(self.raw_data_nodes2, requires_grad=False)
            # shape (B,V+1,2)  customer num + depot
            self.raw_data_capacity2 = torch.tensor(self.raw_data_capacity2, requires_grad=False)
            # shape (B )
            self.raw_data_demand2 = torch.tensor(self.raw_data_demand2, requires_grad=False)
            # shape (B,V+1) customer num + depot
            self.raw_data_cost2 = torch.tensor(self.raw_data_cost2, requires_grad=False)
            # shape (B )
            self.raw_data_node_flag2 = torch.tensor(self.raw_data_node_flag2, requires_grad=False)
            # shape (B,V,2)
