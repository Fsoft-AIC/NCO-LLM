        ...
        if self.mode == 'train':
            ...
            if ATTENTION_BIAS:
                self.attention_bias = torch.stack([
                    heu.heuristics2(distance_matrices[i], demands[i]) for i in range(distance_matrices.size(0))
                ], dim=0)
            ...
            probs = self.decoder(self.encoder(state.problems,self.capacity),
                                 selected_node_list, self.capacity,remaining_capacity, attention_bias=self.attention_bias)
...
class CVRP_Decoder(nn.Module):
    ...
    def forward(self, data,selected_node_list,capacity,remaining_capacity,attention_bias=None):
        ...
        # Attention Bias via LLM: add attention bias
        if ATTENTION_BIAS:
