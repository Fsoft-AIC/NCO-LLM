...
from LEHD.CVRP.VRPModel_lm import VRPModel as Model
...
class VRPTester():
    ...
    def run(self):
        ...
        # episode = 92 #for testing on X dataset
        ...
        while episode < test_num_episode:
            ...
            elif vrpname=='Leuven1':
                problem_Leuven1.append(current_gap)
            elif vrpname=='Leuven2':
                problem_Leuven2.append(current_gap) 
            elif vrpname=='Antwerp1':
                problem_Antwerp1.append(current_gap) 
            elif vrpname=='Antwerp2':
                problem_Antwerp2.append(current_gap) 
            ...
            self.logger.info(" problem_L1    mean gap:{:4f}%, num:{}".format(np.mean( problem_Leuven1)*100, len(problem_Leuven1)))
            self.logger.info(" problem_L2    mean gap:{:4f}%, num:{}".format(np.mean( problem_Leuven2)*100, len(problem_Leuven2)))
            self.logger.info(" problems_A1   mean gap:{:4f}%, num:{}".format(np.mean( problem_Antwerp1)*100, len(problem_Antwerp1)))
            self.logger.info(" problems_A2   mean gap:{:4f}%, num:{}".format(np.mean( problem_Antwerp2)*100, len(problem_Antwerp2)))
        ...
        return score_AM.avg, score_student_AM.avg, 0
    ...
    def _test_one_batch(self, episode, batch_size, clock=None,logger = None):
        ...
        with torch.no_grad():
            ...
            self.optimal_length = torch.tensor(self.optimal_length)
            B_V = batch_size * 1
